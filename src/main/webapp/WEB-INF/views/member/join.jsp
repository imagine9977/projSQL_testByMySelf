	...
	<div class="full-wrap">
		...
		<main id="contents" class="contents">
			<div id="breadcrumb" class="container breadcrumb-wrap clr-fix"
				...
				<nav class="breadcrumb" aria-label="breadcrumbs">
					<ul>
						...
						<li class="is-active"><a href="#" aria-current="page">Join
								Form</a></li>
					</ul>
				</nav>
			</div>
			<section class="page" id="page1">
				...
				<hr>
				<div class="page-wrap">
					<form name="frm1" id="frm1" action="${path2 }/member/joinPro.do"
						method="post" onsubmit="return joinCheck(this)">
						<table id="table1" class="table">
							<tbody>
								<tr>
									<th style="background-color: #dcdcdc">아이디</th>
									<td><input type="text" name="id" id="id"
										placeholder="아이디 입력" pattern="^[a-z0-9]{5,12}" maxlength="12"
										class="input" required style="width: 700px; float: left;">
										...
										name="idck" id="idck" value="no" /> <c:if
											...
											<p id="msg" style="clear: both; padding: 0.5rem">아이디 중복
												체크후 수정하였습니다.</p>
										</c:if></td>
								</tr>
								<tr>
									...
									<td><input type="password" name="pw" id="pw"
										placeholder="비밀번호 입력" maxlength="12" class="input" required>
								...
								<tr>
									...
									<td><input type="password" name="pw2" id="pw2"
										placeholder="비밀번호 확인 입력" maxlength="12" class="input" required>
										<!-- pattern="^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&])[A-Za-z\d$@$!%*#?&]{8,12}$"  -->
									</td>
								</tr>
								<tr>
									...
									<td><input type="text" name="name" id="name"
										placeholder="이름 입력" class="input" maxlength="40" required>
									</td>
								</tr>
								<tr>
									<th style="background-color: #dcdcdc">이메일</th>
									<td><input type="email" name="email" id="email"
										placeholder="이메일 입력" pattern="^\w+((\.\w+)?)+@\w+.?\w+\.\w+$"
										maxlength="80" class="input" required></td>
								</tr>
								<tr>
									<th style="background-color: #dcdcdc">전화번호</th>
									<td><input type="tel" name="tel" id="tel"
										placeholder="전화번호 입력"
										pattern="[0-9]{2,3}-[0-9]{3,4}-[0-9]{3,4}" maxlength="19"
								...
								<tr>
									...
									<td><input type="text" name="addr1" id="addr1"
										...
										type="hidden" name="addrck" id="addrck" value="no" /></td>
								</tr>
								<tr>
													...
													.keyup(
															function() {
																...
																if ($(this)
																	...
																	$("#msg")
																			.html(
																					"<strong>아이디 입력중입니다.</strong>");
																} else {
																	$("#msg")
																			.html(
																					"아직 아이디 중복 체크를 하지 않으셨습니다.");
																}
															});
										});
					</script>
					<script>
						function idCheck() {
							...
							$
									.ajax({
										url : "${path2 }/member/idCheck.do?id="
												+ $("#id").val(), //아이디가 전송되어질 곳
										type : "get", //전송방식
										...
										success : function(result) {
											...
											if (!idChk) { //사용할 수 없는 아이디
												...
												$("#msg")
														.html(
																"<strong style='color:red'>기존에 사용되고 있는 아이디 입니다. 다시 입력하시기 바랍니다.</strong>");
												$("#id").focus();
											} else if (idChk) { //사용 가능한 아이디
												$("#idck").val("yes");
												$("#msg")
														.html(
																"<strong style='color:blue'>사용가능한 아이디 입니다.</strong>");
											} else if (idck == "") {
												$("#msg")
														.html(
																"<strong>아이디가 확인되지 않았습니다. 다시 시도해주시기 바랍니다.</strong>");
											}
										}
									});
						}
							...
							new daum.Postcode(
									{
										oncomplete : function(data) {
											...
											if (roadAddr !== '') {
												document
														.getElementById("addr1").value = roadAddr;
											} else if (jibunAddr !== '') {
												document
														.getElementById("addr1").value = jibunAddr;
											}
